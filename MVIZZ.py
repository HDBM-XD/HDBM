Author = 'HDBM_XD'
Status = 'Free'
Note = 'Jangan Jual Script Ini @$✓'
WahaiPenjualScript = 'kapret'
...
▉◥◤▉ ◥◣◢◤ ▉ ▉◤┊ ▉◤┊
▉┆┆▉ ┆◥◤┆ ▉ ▉▂▉ ▉▂▉
RECODE BY HDBM×MVIZZ.ME{Note}{X}
"""
...
def login_cookie():
	clearlayar()
	cok = input(' Input Cookie OMM : ')
	try:
		head = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36"}
		link = ses.get("https://web.facebook.com/adsmanager?_rdc=1&_rdr", headers = head, cookies = {"cookie": cok})
		find = re.findall('act=(.*?)&nav_source', link.text)
		if len(find) == 0:print(' Login Gagal OMM, Ganti cookie ');time.sleep(2);exit()
		else:
			for JK in find:
...
def MainMenu():
	...
	except IOError:print(' Cookies Kedaluwarsa TUMBALIN AKUN MU');login_cookie()
	clearlayar()
